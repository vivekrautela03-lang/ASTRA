"""
Drop-in ASTRA plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — ASTRA discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the AstraUI instance — use player.write_log(f"ASTRA: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"ASTRA: {result_text}")
        except Exception:
            pass
    return result_text
