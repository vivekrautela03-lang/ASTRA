...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".astra_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".astra_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ASTRA profile")
                astra = str(Path.home() / ".astra_profiles" / "firefox_astra")
                Path(astra).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(astra, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".astra_profiles" / "safari")
        ...
        # ASTRA otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        astra_profile = str(Path.home() / ".astra_profiles" / self.browser_name)
        Path(astra_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with ASTRA profile: {astra_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(astra_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with ASTRA profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "astra_screenshot.png")
