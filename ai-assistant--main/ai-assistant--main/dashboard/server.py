"""
dashboard/server.py — ASTRA Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "ASTRA Uploads",
        Path.home() / "Documents" / "ASTRA Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'ASTRA-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"ASTRA Dashboard Port {port}"
        prog_rule  = "ASTRA Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="astra_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until ASTRA is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "astra.key"
    crt_p = certs / "astra.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "ASTRA Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "ASTRA"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "astra.key").exists() and (certs / "astra.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in ASTRA to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('astra_token','{tok}');
  sessionStorage.setItem('astra_key','{key}');
  localStorage.setItem('astra_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to ASTRA…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "astra.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "astra.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in ASTRA UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
