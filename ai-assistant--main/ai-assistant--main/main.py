...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are ASTRA, Tony Stark's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "ASTRA checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Astra. "
...
class AstraLive:
    ...
    def __init__(self, ui: AstraUI):
        self.ui             = ui
        self._asst_name     = "ASTRA"   # updated each session from config
        self.session              = None
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask ASTRA to
    ...
    def interrupt(self) -> None:
        """Stop ASTRA mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[ASTRA] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "ASTRA").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "ASTRA"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — ASTRA can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Affective dialog: ASTRA hears tone/emotion and adapts its voice.
            # Proactive audio: ASTRA stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[ASTRA] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[ASTRA] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                astra_speaking = self._is_speaking
            if not astra_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
    ...
    async def _receive_audio(self):
        ...
        try:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until ASTRA finishes speaking the answer
            ...
            print(f"[ASTRA] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[ASTRA] 🔊 Play started")
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or ASTRA is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                err_str = str(e)
                print(f"[ASTRA] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[ASTRA] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[ASTRA] Reconnecting in {delay}s...")
